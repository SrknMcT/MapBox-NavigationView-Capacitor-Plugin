...
import com.routenavigation.databinding.MapboxActivityTurnByTurnExperienceBinding
import java.util.*
...
class TurnByTurnExperienceActivity : AppCompatActivity(), PermissionsListener {
    ...
    private val routeProgressObserver = RouteProgressObserver { routeProgress ->
        ...
        if (routeProgress.currentState == RouteProgressState.COMPLETE)
            navigationStatus = "navigationCompleted"
        if (routeProgress.currentState == RouteProgressState.INITIALIZED)
            navigationStatus = "navigationInitialized"
        if (routeProgress.currentState == RouteProgressState.TRACKING)
            navigationStatus = "navigationTracking"
        if (routeProgress.currentState == RouteProgressState.OFF_ROUTE)
            navigationStatus = "navigationOffRoute"
        if (routeProgress.currentState == RouteProgressState.UNCERTAIN)
            navigationStatus = "navigationUncertain"


        //Log.i("NAVIGATION", navigationStatus)

        Implementation.getInstance().getNavigationResultCallbackImpl()
            .onStatusChanged(navigationStatus)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val startToParse: String = getIntent().getStringExtra("pointStartToParse").toString()
        val destToParse: String = getIntent().getStringExtra("pointEndToParse").toString()

        val latlongStart = startToParse.split(",").toTypedArray()
        val latlongDest = destToParse.split(",").toTypedArray()

        startLatitude = latlongStart[0].toDouble()
        startLongitude = latlongStart[1].toDouble()
        ...
        binding.stop.setOnClickListener {
            ...
            navigationStatus = "navigationCancelled"
            Implementation.getInstance().getNavigationResultCallbackImpl()
                .onStatusChanged(navigationStatus)
        }
            ...
            mapboxReplayer.pushEvents(
                listOf(
                    ReplayRouteMapper.mapToUpdateLocation(
                        eventTimestamp = 0.0,
                        point = Point.fromLngLat(startLongitude, startLatitude) //darıca
    ...
    private fun clearRouteAndStopNavigation() {
        ...
        // Log.i("NavigationChecks", "NavigationCancelled")
    }
    ...
    override fun onExplanationNeeded(permissionsToExplain: MutableList<String>?) {
        Toast.makeText(this, "Navigation needs your gps permission", Toast.LENGTH_SHORT)
    }
    ...
    override fun onPermissionResult(granted: Boolean) {
        if (granted) {
            ...
            Toast.makeText(this, "User denied permission", Toast.LENGTH_SHORT)
